...
def get_learning_objectives(text):
    ...
    if not learning_objectives or len(learning_objectives) <= 0:
        return {"error":"Sorry no objectives found for " + course_section_name}
    else:
        response = []
        for l_o in learning_objectives:
            response.append(l_o['description'])

        return {"error":"Here are the learning objectives for " + course_section_name + ": " + json.dumps(response, cls=DateTimeEncoder)}
