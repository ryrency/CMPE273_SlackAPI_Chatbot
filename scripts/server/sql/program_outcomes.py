...
def get_program_outcomes(text):
    ...
    if not program_outcomes or len(program_outcomes) <= 0:
        return {"error":"Sorry no program outcomes found for " + course_section_name}
    else:
        response = []
        for p_o in program_outcomes:
            response.append(p_o['description'])

        return {"error":"Here are the program outcomes for " + course_section_name + ':' + json.dumps(response, cls=DateTimeEncoder)}
