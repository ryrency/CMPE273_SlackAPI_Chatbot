...
def get_lab_schedule(text):
    ...
    if not lab_schedules or len(lab_schedules) <= 0:
        return {"error":"Sorry no lab schedules found for " + course_section_name}
    else:
        ...
        if seq_no > 0 and seq_no <= len(lab_schedules):
            ...
            return {"error":"Sorry, don't know which lab schedule you're looking for. But here is a schedule for all labs of " + course_section_name + ": " + json.dumps(all_labs_schedule, cls=DateTimeEncoder)}
